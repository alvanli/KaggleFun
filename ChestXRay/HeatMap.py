...
def permutation_importance(X, y, model, metric, num_samples = 100):
    ...
    for feature in range(len(X.columns)): # complete this line
        ...
        for i in range(num_samples): # complete this line
            ...
            perm_X = permute_feature(X, X.columns[feature])
        ...
        importances[X.columns[feature]]['importance'] = np.abs(np.mean(feature_performance_arr)-np.mean(baseline_performance))
