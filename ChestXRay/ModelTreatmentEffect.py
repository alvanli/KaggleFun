...
def event_rate(df):
    ...
    treated_prob = np.sum(df[df["TRTMT"] == 1]["outcome"])/np.sum(df["TRTMT"])
    control_prob = np.sum(df[df["TRTMT"] == 0]["outcome"])/np.sum(df["TRTMT"] == 0)
...
def lr_ARR_quantile(X, y, lr):
    ...
    arr_by_risk_group.index = baseline_mean_by_risk_group
...
def c_statistic(pred_rr, y, w, random_seed=0):
    ...
    tuples = [(pred_rrr, yy, ww) for pred_rrr, yy, ww in zip(pred_rr, y, w)]
    
    # Collect untreated patient tuples, stored as a list
    untreated = [(pred_rrr, yy, ww) for pred_rrr, yy, ww in zip(pred_rr, y, w) if ww == 0]
    
    # Collect treated patient tuples, stored as a list
    treated = [(pred_rrr, yy, ww) for pred_rrr, yy, ww in zip(pred_rr, y, w) if ww == 1]
